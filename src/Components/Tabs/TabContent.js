...
export default function TabContent({ children }) {
	return (
		<React.Fragment>
			{children.map((item) => (
				<span
					className={`grid transition-opacity delay-100 ease-in-out duration-300 gap-x-6  gap-y-6 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4  ${item.isActive ==
					true
						? 'block opacity-100'
						: 'hidden opacity-0'}`}
					key={item.name}
